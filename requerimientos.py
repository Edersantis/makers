...
if st.session_state.tickets:
    ...
    with st.expander("Ver gráficos de distribución"):
        st.subheader("Distribución por estado")
        fig, ax = plt.subplots()
        estado_counts = df["Estado"].value_counts()
        ax.bar(estado_counts.index, estado_counts.values)
        ...
        ax2.pie(prioridad_counts.values, labels=prioridad_counts.index, autopct='%1.1f%%', startangle=90)
        ax2.axis('equal')
        st.pyplot(fig2)
else:
